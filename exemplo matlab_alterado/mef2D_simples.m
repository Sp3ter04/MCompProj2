...
face_force = find(gcoord(:, 1) == 2); % Ir buscar nos que estão na zona 
% da direita da placa para aplicarmos a força

% Carregamento pontual nos nos
P(1:sdof)=0;
P(face_force*2-1)=1e5; % Aplicar força ao grau de liberdade em x dos nos do
% face_force
...
for iel=1:nelem
  ...
  f=zeros(12,1); % as forças distribuidas vão ser aplicadas nos nós (forças equivalentes nodais)
  index=eldof(iel,nnel,nodes,gdl);
  [kk,ff]=assemb(kk,ff,k,f,index);
end

%% Parece me safe (talvez não)
% imposicao de condicoes de fronteira
[kk,ff]=cf(kk,ff,bcdof,bcval); % O professor atualizou isto

ff=ff+P; % imposicao de cargas pontuais
